...
export function DonationModal({
  family, amount, cardNumber, onClose, onConfirm,
}: DonationModalProps) {
  const displayCard = cardNumber ?? DEMO_CARD
  ...
  function copyCard() {
    navigator.clipboard.writeText(displayCard).catch(() => {})
    setCopied(true)
          ...
          {step === 'success' && (
            <div className="flex flex-col items-center text-center gap-5 py-4">
              ...
              <div
                ...
                style={{ backgroundColor: 'oklch(0.97 0.018 258)', border: '1.5px solid oklch(0.91 0.025 258)' }}
              >
                <p className="text-xs font-black uppercase tracking-wider mb-1" style={{ color: BLUE }}>
                  Get more from your giving
                </p>
                <p className="text-sm leading-relaxed mb-3" style={{ color: 'oklch(0.42 0.07 258)' }}>
                  Create a free account to track your donations and receive updates from the families you've supported.
                </p>
                <div className="flex gap-2">
                  <a
                    href="/signup"
                    className="flex-1 py-2.5 rounded-full text-xs font-black text-center text-white transition-all"
                    style={{ backgroundColor: BLUE }}
                    onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.backgroundColor = 'oklch(0.30 0.14 260)' }}
                  ...
                  <a
                    href="/login"
                    className="flex-1 py-2.5 rounded-full text-xs font-black text-center transition-all"
                    style={{ backgroundColor: 'white', color: BLUE, border: `1.5px solid ${BLUE}` }}
                    onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.backgroundColor = 'oklch(0.95 0.022 258)' }}
